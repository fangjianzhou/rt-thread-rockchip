...
struct rt_ofw_node *rt_ofw_node_get(struct rt_ofw_node *np)
{
    if (np)
    {
        LOG_D("%s get ref = %d", np->full_name, ref_read(&np->ref));
        ref_get(&np->ref);
    }
...
static void ofw_node_release(struct ref *r)
{
...
void rt_ofw_node_put(struct rt_ofw_node *np)
{
    if (np)
    {
        LOG_D("%s put ref = %d", np->full_name, ref_read(&np->ref));
        ref_put(&np->ref, &ofw_node_release);
    }
}
...
{
    struct rt_ofw_node *np = RT_NULL, *parent, *tmp;
...
rt_err_t ofw_alias_scan(void)
{
    ...
    rt_ofw_foreach_prop(np, prop)
    {
        ...
        const char *name = prop->name, *end, *ch;
...
static const char *ofw_get_prop_fuzzy_name(const struct rt_ofw_node *np, const char *name)
{
    ...
    DECLARE_BITMAP(ak, sizeof(split_field)) = {};
    struct rt_ofw_prop *prop;
    ...
    *sf = '\0';
    ...
    rt_ofw_foreach_prop(np, prop)
    {
        ...
        if (!bitmap_test_bit(ak, prep_ak))
        {
            next_ak = bitmap_next_set_bit(ak, prep_ak + 1, max_ak) ? : len;
        ...
        bitmap_for_each_set_bit_from(ak, prep_ak, next_ak, max_ak)
        {
            /* Skip the '*' */
            if (prep_ak == next_ak)
            {
                ...
                next_ak = bitmap_next_set_bit(ak, prep_ak + 1, max_ak);
            }
