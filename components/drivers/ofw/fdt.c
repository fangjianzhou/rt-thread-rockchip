...
static void abandon_usable_memregion(rt_region_t *region)
{
    rt_region_t *region_next = region + 1;
    int idx = region - &_memregion[0], last = _memregion_front_idx - 1;

    if (last > idx)
    {
        rt_memmove(region, region_next, sizeof(rt_region_t) * (last - idx));
    }

    _memregion[last].name = RT_NULL;
    --_memregion_front_idx;
}
    ...
    for (no = 0; region->name; ++region)
    {
        ...
        for (int i = RT_ARRAY_SIZE(_memregion) - 1; i > no; --i)
        {
            ...
            if (res_region->start >= region->start && res_region->end <= region->end)
            {
                ...
                {
                    abandon_usable_memregion(region);
                }
...
rt_err_t rt_fdt_scan_initrd(rt_uint64_t *ranges)
{
    ...
    if (_fdt && ranges)
    {
        ...
        if (err)
        {
            ...
            if (bootargs && (options = rt_strstr(bootargs, "initrd=")))
            {
                rt_uint64_t value;

                options += sizeof("initrd=0x") - 1;
                err = RT_EOK;
