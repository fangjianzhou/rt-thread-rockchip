...
static struct rt_object *ofw_parse_object(struct rt_ofw_node *np, const char *cells_name)
{
    ...
    DECLARE_BITMAP(idx_mask, RT_ARRAY_SIZE(ofw_obj_cmp_list)) = {};
    ...
    while (!ret_obj)
    {
        ...
        bitmap_for_each_clear_bit(idx_mask, i, RT_ARRAY_SIZE(ofw_obj_cmp_list))
        {
            item = &ofw_obj_cmp_list[i];
        ...
        if (rt_unlikely(i >= RT_ARRAY_SIZE(ofw_obj_cmp_list)))
        {
