...
static DECLARE_BITMAP(msi_irq_map, MAX_HANDLERS) = {};
    ...
    _retry:
        for (int i = 0; i < irq_nr; ++i)
        {
            ...
            if (last_irq >= 0 && last_irq + 1 != irq)
            {
                ...
                {
                    bitmap_set_bit(msi_irq_map, last_irq);
                }
        ...
        bitmap_for_each_set_bit(msi_irq_map, irq, MAX_HANDLERS)
        {
            msi_pic->ops->irq_free_msi(msi_pic, irq);

            /* Free bit so the next user doesn't need to bzero */
            bitmap_clear_bit(msi_irq_map, irq);
        }
