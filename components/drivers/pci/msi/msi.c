...
static RT_DECLARE_IRQ_AFFINITY(msi_affinity_default[2048]) rt_section(".bss.noclean.pci.msi");
        ...
        bitmap_t *cpumasks)
{
    ...
    else if (bitmap_next_set_bit(cpumasks, 0, RT_CPUS_NR) >= RT_CPUS_NR)
    {
...
rt_ssize_t rt_pci_alloc_vector(struct rt_pci_device *pdev, int min, int max,
        rt_uint32_t flags, RT_DECLARE_IRQ_AFFINITY((*affinities)))
{
    ...
    if (flags & RT_PCI_IRQ_F_LEGACY)
    {
        if (min == 1 && pdev->irq >= 0)
        {
            ...
            {
                int cpuid;
                RT_DECLARE_IRQ_AFFINITY(old_affinity);

                /* INTx is shared, we should update it */
                rt_pic_irq_get_affinity(pdev->irq, old_affinity);

                bitmap_for_each_set_bit(affinities[0], cpuid, RT_CPUS_NR)
                {
...
static rt_ssize_t msi_capability_init(struct rt_pci_device *pdev,
        int nvec, RT_DECLARE_IRQ_AFFINITY((*affinities)))
{
    rt_err_t err;
    struct rt_pci_msi_desc *desc;
...
rt_ssize_t rt_pci_msi_enable_range_affinity(struct rt_pci_device *pdev,
        int min, int max, RT_DECLARE_IRQ_AFFINITY((*affinities)))
{
    int nvec = max;
...
static rt_ssize_t msix_capability_init(struct rt_pci_device *pdev,
        ...
        RT_DECLARE_IRQ_AFFINITY((*affinities)))
{
    rt_err_t err;
    rt_uint16_t msgctl;
