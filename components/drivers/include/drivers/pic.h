...
struct rt_pic_ops
{
    ...
    rt_err_t    (*irq_set_affinity)(struct rt_pic_irq *pirq, bitmap_t *affinity);
    rt_err_t    (*irq_set_triger_mode)(struct rt_pic_irq *pirq, rt_uint32_t mode);

    void        (*irq_send_ipi)(struct rt_pic_irq *pirq, bitmap_t *cpumask);
...
#define RT_DECLARE_IRQ_AFFINITY(name)           DECLARE_BITMAP(name, RT_CPUS_NR)
#define RT_IRQ_AFFINITY_SET(affinity, cpuid)    bitmap_set_bit(affinity, cpuid)
#define RT_IRQ_AFFINITY_CLEAR(affinity, cpuid)  bitmap_clear_bit(affinity, cpuid)
...
#define RT_IRQ_MODE_MASK            0xf
    ...
    RT_DECLARE_IRQ_AFFINITY(affinity);
...
rt_err_t rt_pic_irq_set_affinity(int irq, bitmap_t *affinity);
rt_err_t rt_pic_irq_get_affinity(int irq, bitmap_t *out_affinity);
rt_err_t rt_pic_irq_set_triger_mode(int irq, rt_uint32_t mode);
rt_uint32_t rt_pic_irq_get_triger_mode(int irq);
void rt_pic_irq_send_ipi(int irq, bitmap_t *cpumask);
...
rt_err_t rt_pic_irq_parent_set_affinity(struct rt_pic_irq *pirq, bitmap_t *affinity);
