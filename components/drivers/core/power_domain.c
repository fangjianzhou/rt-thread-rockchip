...
{
    return ref_read(&domain->ref) == 1 && !rt_list_isempty(&domain->child_nodes);
}
...
static void dm_power_domain_release(struct ref *r)
{
...
rt_err_t rt_dm_power_domain_power_off(struct rt_dm_power_domain *domain)
{
    ...
    ref_put(&domain->ref, dm_power_domain_release);
