...
{
    rt_uint32_t val, len = i2c->msg->len - i2c->processed;
...
static struct rt_i2c_bus_device_ops rk3x_i2c_ops =
{
    .master_xfer = rk3x_i2c_master_xfer,
};

static void rk3x_i2c_irq(int irqno, void *param)
{
    rt_uint32_t ipd;
...
static rt_err_t rk3x_i2c_probe(struct rt_platform_device *pdev)
{
    ...
    if (i2c->soc_data->grf_offset >= 0)
    {
        ...
        int id = rt_ofw_get_alias_id(np, "i2c");
    ...
    if (!i2c->clk || rt_clk_prepare(i2c->clk))
    {
        err = -RT_EIO;
    ...
    if (!i2c->pclk || rt_clk_prepare(i2c->pclk))
    {
        err = -RT_EIO;
    ...
    rt_hw_interrupt_install(i2c->irq, rk3x_i2c_irq, i2c, dev_name);
    rt_hw_interrupt_umask(i2c->irq);

    i2c->parent.ops = &rk3x_i2c_ops;
    i2c->parent.parent.ofw_node = dev->ofw_node;

    rt_i2c_bus_device_register(&i2c->parent, dev_name);
