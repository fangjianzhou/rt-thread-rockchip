...
#define raw_to_fiq_debugger(raw) rt_container_of(raw, struct rk_fiq_debugger, parent)

rt_inline void rk_fiq_write(struct rk_fiq_debugger *t, rt_uint32_t val, int off)
{
    HWREG32(t->debug_port_base + off * 4) = val;
}

rt_inline rt_uint32_t rk_fiq_read(struct rk_fiq_debugger *t, int off)
{
    return HWREG32(t->debug_port_base + off * 4);
}

rt_inline rt_uint32_t rk_fiq_read_lsr(struct rk_fiq_debugger *t)
{
    rt_uint32_t ret = rk_fiq_read(t, UART_LSR);
...
static void fiq_debugger_isr(int irqno, void *param)
{
    ...
    struct rk_fiq_debugger *t = raw_to_fiq_debugger(serial);
...
static rt_err_t fiq_debugger_uart_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    /* reset uart */
    rk_fiq_write(t, (1 << 1) | (1 << 2), UART_SRR);
    rt_hw_us_delay(10);
...
static rt_err_t fiq_debugger_uart_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CLR_INT:
        rt_hw_interrupt_mask(t->irq);
        break;

    case RT_DEVICE_CTRL_SET_INT:
        rt_hw_interrupt_umask(t->irq);
        break;
    }
...
struct rk_fiq_debugger *rk_serial_debug_init(void *base, rt_ubase_t paddr,
    int irq, int signal_irq, int wakeup_irq, rt_uint32_t baudrate)
{
    struct rk_fiq_debugger *t = rt_calloc(1, sizeof(*t));
    ...
    if (t)
    {
        ...
        rt_hw_interrupt_install(t->irq, fiq_debugger_isr, &t->parent, name);
    }

    return t;
}

static rt_err_t rk_fiq_debugger_probe(struct rt_platform_device *pdev)
{
    rt_err_t err = RT_EOK;
    void *base;
    rt_uint64_t regs[2];
    ...
    {
        struct rk_fiq_debugger *t = rk_serial_debug_init(base,
...
static const struct rt_ofw_node_id rk_fiq_debugger_ofw_ids[] =
{
    { .type = "ttyFIQ", .compatible = "rockchip,fiq-debugger" },
    { /* sentinel */ }
};

static struct rt_platform_driver rk_fiq_debugger_driver =
{
    .name = "rk-fiq-debugger",
    .ids = rk_fiq_debugger_ofw_ids,

    .probe = rk_fiq_debugger_probe,
};

static int rk_fiq_debugger_drv_register(void)
{
    rt_platform_driver_register(&rk_fiq_debugger_driver);

    return 0;
}
INIT_DRIVER_EARLY_EXPORT(rk_fiq_debugger_drv_register);
